...
If the teachers say: *"Open your code and show us exactly where you wrote this!"* — don't panic! Here is your ultimate cheat sheet. It tells you exactly which file to open, which function to point at, and how to explain the logic in easy words!

### 1. The Short-Term Memory (React Hooks)
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx`
* **Which Function to point at:** Point to `const [fen, setFen] = useState(...)` and `useEffect(...)` near the top.
* **How to explain the logic:** "We used React Hooks! `useState` is like the computer's short-term memory—it remembers whose turn it is and how much time is left. `useEffect` is a rule that says 'When X changes, automatically do Y' (like when the turn changes, it automatically triggers a rule to check if we should flip the board)."

### 2. The Countdown Timers
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx`
* **Which Function to point at:** Point to the `useEffect` block that contains the word `setInterval()`.
* **How to explain the logic:** "We wrote a `setInterval` loop that triggers every 1000 milliseconds (1 second). It checks a boolean `isWhiteTurn`. If true, it grabs White's timer number and subtracts 1. If the timer hits exactly `0`, we instantly call the `handleTimeout()` function to end the game!"

### 3. Move History (The Game Story)
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx`
* **Which Function to point at:** Point to the `groupedHistory.map(...)` code near the bottom.
* **How to explain the logic:** "Every time a valid move is played, we use `.push()` to add the move's string (like 'e4') to our `moveHistory` Array. Down here in the UI, we use the JavaScript `.map()` function. `.map()` works exactly like a printing press—it loops over every single item in our Array and prints a brand new HTML row on the screen to show the move."

### 4. Material Advantage (Counting Pieces)
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx`
* **Which Function to point at:** Point to the `function getMaterial(fen)` near the top.
* **How to explain the logic:** "The board is saved as a long string of letters called a FEN (where 'Q' is a White Queen and 'q' is a Black queen). Our `getMaterial` function reads this string character by character. If it sees a 'Q' is missing from the starting count, it looks up the Queen in our point Object (worth 9 points) and gives 9 points to Black. It subtracts the totals to find out exactly who is winning mathematically!"

### 5. Resign and Draw Buttons
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx`
* **Which Function to point at:** Point to the `const handleResign = () => { ... }` function.
* **How to explain the logic:** "We attached an `onClick` event listener to the button. When clicked, it fires the `handleResign` function. Inside this function, we write a new string over the game's `status` variable. For example, if White resigns, we change the status string to `black_wins_by_resignation`. The React UI sees this new string and instantly locks the board so no one can play!"

### 6. Rotating the Board
* **Where to find it (File):** Open `frontend/src/components/ChessBoard.tsx`
* **Which Function to point at:** Point to the `<ChessBoard isFlipped={isFlipped} />` component.
* **How to explain the logic:** "We pass a boolean flag called `isFlipped` into the board. Inside the `ChessBoard` file, we draw the 64 squares using a `for` loop. If `isFlipped` is true, we reverse the math in our loop so it draws row 1 at the top instead of the bottom. We also swap the position of the Player Cards in the layout based on this exact same boolean!"

### 7. The C++ Brain API
* **Where to find it (File):** Open `frontend/src/app/play/page.tsx` AND `frontend/src/app/api/chess/route.ts`
* **Which Function to point at:** Point to the `handleMove` function where it says `fetch('/api/chess', ...)`
* **How to explain the logic:** "This is our Node.js Secret Mailman! When a player tries to move, we send a JSON package to `/api/chess` containing the starting square and ending square. Node.js sends this to our custom C++ game engine, which uses complex math to check if the path is blocked. The C++ engine replies 'valid' or 'invalid', and Node.js hands that answer right back to React!"
